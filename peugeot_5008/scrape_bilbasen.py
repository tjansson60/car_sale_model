...
url = '''https://www.bilbasen.dk/brugt/bil/Peugeot/5008?
    fuel=0&
    yearfrom=0&
    yearto=0&
    pricefrom=0&
...
results = []
for page in tqdm.tqdm(range(1, 5)):  # 492/32 = 15.375
...
df.loc[df['headline'].str.contains('e-THP'), 'fuelType'] = 'e-THP'
...
df.loc[df['headline'].str.contains('Premium'), 'trim']        = 'Premium'
df.loc[df['headline'].str.contains('Allure'), 'trim']         = 'Allure'
df.loc[df['headline'].str.contains('Strike'), 'trim']         = 'Strike'
df.loc[df['headline'].str.contains('GT'), 'trim']             = 'GT'
